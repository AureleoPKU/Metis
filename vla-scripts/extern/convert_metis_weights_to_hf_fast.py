...
def convert_dexvla_weights_to_hf(cfg: HFConvertConfig) -> None:
    ...
    elif cfg.lamtype == "Fast_Tokenizer":
        special_tokens_dict = {'additional_special_tokens': ['<BOA>', '<BOR>'] + [f'<DYN_{i}>' for i in range(cfg.codebook_size)] + [f'<DEX_{i}>' for i in range(cfg.motion_codebook_size)] + [f'<robot_action_{i}>' for i in range(2048)]}
    else:
    ...
    # Some checkpoints use a smaller tokenizer vocab than the one constructed here (e.g. extra
    # "Fast_Tokenizer" special tokens). To avoid `size mismatch` during conversion, reconcile the
    # checkpoint embedding matrices with the target model shapes by copying the overlapping rows and
    # keeping the remaining rows as randomly initialized.
    _embed_key = "language_model.model.embed_tokens.weight"
    _lm_head_key = "language_model.lm_head.weight"

    target_embed = hf_model.get_input_embeddings().weight
    ckpt_embed = converted_state_dict[_embed_key]
    if ckpt_embed.shape != target_embed.shape:
        tgt_rows = target_embed.shape[0]
        ckpt_rows = ckpt_embed.shape[0]
        n = min(tgt_rows, ckpt_rows)
        new_embed = target_embed.detach().clone()
        new_embed[:n] = ckpt_embed[:n].to(new_embed.dtype)
        converted_state_dict[_embed_key] = new_embed

    if _lm_head_key in converted_state_dict:
        target_lm_head = hf_model.get_output_embeddings().weight
        ckpt_lm_head = converted_state_dict[_lm_head_key]
        if ckpt_lm_head.shape != target_lm_head.shape:
            tgt_rows = target_lm_head.shape[0]
            ckpt_rows = ckpt_lm_head.shape[0]
            n = min(tgt_rows, ckpt_rows)
            new_lm_head = target_lm_head.detach().clone()
            new_lm_head[:n] = ckpt_lm_head[:n].to(new_lm_head.dtype)
            converted_state_dict[_lm_head_key] = new_lm_head
