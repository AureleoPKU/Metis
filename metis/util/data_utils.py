...
@dataclass
class CollatorForMotionDynamics:
    # vgg_transform: StreamVGGT
    pixel_values_dtype: torch.dtype = torch.float32
    ...
    def __call__(self, instances: Sequence[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        videos = []
        for instance_pixel_values in all_pixel_values:

            video_frames = torch.stack(instance_pixel_values)  # [num_frames, channels, height, width]
            videos.append(video_frames)


        pixel_values = torch.stack(videos)  # [batch_size, num_frames, channels, height, width]
        ...
        if "last_images_processed" in instances[0]:
            last_images_processed = [instance["last_images_processed"] for instance in instances]
            last_images_tensor = []
            for instance_last_images_tensor in last_images_processed:
                last_images_tensor.append(torch.stack(instance_last_images_tensor))
            last_images_tensor = torch.stack(last_images_tensor, dim=0)
        else:
            last_images_tensor = None
        ...
        output = dict(
            ...
            task_instruction=task_instruction,
            action=action,
            last_images_tensor=last_images_tensor,
        )
