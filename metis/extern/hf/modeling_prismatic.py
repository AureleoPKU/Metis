...
from metis.models.policy.transformer_utils import MAPBlock, MAPBlockStack
from metis.models.policy_heads.models.unet_diffusion.modeling_unet_diffusion import ConditionalUnet1D
from .configuration_prismatic import PrismaticConfig, DexVLAConfig
...
class DexVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def __init__(self, config: DexVLAConfig) -> None:
        ...
        if config.use_diffusion_head :
            self.action_decoder = ConditionalUnet1D(input_dim=self.input_dim, window_size=config.window_size, vis_dim=4096)
        elif config.use_flow_matching:
            raise NotImplementedError("Flow matching action decoder is not supported in DexVLAForActionPrediction.")
        else:
            self.action_decoder = ActionDecoder(input_dim=48, window_size=config.window_size, vis_dim=4096, hidden_dim=512)
        ...
        self.num_patches = self.vision_backbone.get_num_patches()
        if self.use_proprio:
            self.num_patches += 1
        
        self.motion_dynamics = None

    def vlm_forward(self, *args, **kwargs):
    ...
    def predict_latent_action(
        ...
        # output = self.generate(input_ids, min_new_tokens=1540, max_new_tokens=1540, return_dict_in_generate=True, output_hidden_states=True, **kwargs)
        # output = self.generate(input_ids, min_new_tokens=142, max_new_tokens=142, return_dict_in_generate=True, output_hidden_states=True, **kwargs)
        ...
        # print(generated_ids)
        latent_mask = generated_ids[0] > 32002
        # for rebutal
        action_mask = generated_ids[0] > 31743
        print("generate:", generated_ids[0])
        # print(latent_mask[0])
        # latent_action = latent_tokens[:, latent_mask[0], :]
        if latent_mask.any():
            print("latent_mask:", latent_mask.sum())
            ...
            print("latent_mask:", latent_mask.sum())
            true_indices = torch.where(action_mask)[0]
            start_index = true_indices[0]
            per_sample_action_tokens = latent_tokens[0][start_index:start_index+48*32, :]
            action_tokens = per_sample_action_tokens.unsqueeze(0).to(torch.float)
            self.motion_dynamics = action_tokens.cpu().numpy()
    ...
    def action_decoder_forward(self, batch, slow_output, diff_loss_mask):
        ...
        mask1 = action_gt > 31743 # Navie tokenizer
        mask2 = action_gt > 32530 # fast tokenizer
        ...
        for idx, per_sample_latent_tokens in enumerate(latent_tokens):
            ...
            m2 = mask2[idx]
            if m2.any():
                true_indices = torch.where(m2)[0]
                start_index = true_indices[0]
                per_sample_action_tokens = per_sample_latent_tokens[start_index:start_index+48, :]
            ...
            # for rebuttal
                ...
                if idx == 0:
                    print("action tokens:", m1.sum())
                true_indices = torch.where(m1)[0]
                # start_index = true_indices[7] + 2
                start_index = true_indices[0]
                per_sample_action_tokens = per_sample_latent_tokens[start_index:start_index+48*32, :]
                action_tokens.append(per_sample_action_tokens)
            else:
        ...
        if self.use_diffusion_head:
            ...
            # print("u_t:", u_t[0])
            # print("v_pred:", v_pred[0])
            # loss_batch = torch.nn.functional.l1_loss(v_pred, u_t, reduction='none')
