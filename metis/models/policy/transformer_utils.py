    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            MAPBlock(
                ...
                n_heads=n_heads,
                mlp_ratio=mlp_ratio,
                do_rms_norm=do_rms_norm,
                do_swish_glu=do_swish_glu,
            )
            for _ in range(depth)
        ])

    def forward(self, x, mask=None, init_embed=None):
        out = self.blocks[0](x, mask=mask, init_embed=init_embed)
        for blk in self.blocks[1:]:
            out = blk(x, mask=mask, init_embed=out)
