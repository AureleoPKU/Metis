...
from metis.models.vlms import PrismaticVLM
from metis.util.nn_utils import FusedMLPProjector, LinearProjector, MLPProjector
from metis.models.backbones.llm import LLMBackbone
from metis.models.backbones.llm.prompting import PromptBuilder
from metis.models.backbones.vision import VisionBackbone
...
class DexVLA_Model(torch.nn.Module):
    def __init__(self, vlm:PrismaticVLM, use_diffusion_head=True, window_size=12):
        super().__init__()
        self.vlm = vlm
        self.window_size = window_size
        self.use_diffusion_head = use_diffusion_head
        if self.use_diffusion_head :
            self.action_decoder = ConditionalUnet1D(input_dim=48, window_size=self.window_size, vis_dim=4096)
        else:
            self.action_decoder = ActionDecoder(window_size=self.window_size, vis_dim=4096)
        self.all_module_keys = ["vlm.vision_backbone", "vlm.llm_backbone", "vlm.projector", "action_decoder"]
    ...
    def get_fsdp_wrapping_policy(self):
        ...
        # --- ① 原有 VLM 策略：把 VLM 里大的块（Transformer blocks、ConvNeXt 等）包进去
        ## 假设你已有 vlm_fsdp_wrapping_policy，可直接引用
        vision_llm_projector_policy = self.vlm.get_fsdp_wrapping_policy()

        # --- ② ActionDecoder 策略：若它比较大，就单独再包一层
