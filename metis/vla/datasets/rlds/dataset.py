...
from metis.vla.datasets.rlds import obs_transforms, traj_transforms
from metis.vla.datasets.rlds.utils import goal_relabeling, task_augmentation
...
def make_dataset_from_cot_rlds(
    ...
    *,
    ...
    if not subdirs:
        raise ValueError(f"目录为空：{parent_dir}")
    def make_restructure(ds_name, reasoning_dict):
        def restructure(traj):
            ...
            def reasoning_for_traj(ep_idx_tensor, frame_idx_tensor):
                ...
                out = np.empty(frames.shape[0], dtype=object)                   # 可变长
                out[:]  = b"" 
