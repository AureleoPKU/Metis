...
from metis.vla.datasets.rlds.oxe.utils.droid_utils import droid_baseact_transform, droid_finetuning_transform
from metis.vla.datasets.rlds.utils.data_utils import (
...
def quaternion_to_6d_tf(quat_xyzw: tf.Tensor) -> tf.Tensor:
    """
    quat_xyzw: [T, 4]，分量顺序为 [x, y, z, w]（若你的数据是 wxyz，先重排）
    返回: [T, 6]，取旋转矩阵的前两列 (r00,r10,r20, r01,r11,r21)
    """
    ...
    rot6d.set_shape([None, 6])   # 很重要：给出静态形状
    return rot6d


# def quaternion_to_6d_scipy(quat: tf.Tensor) -> tf.Tensor:
#     @tf.py_function(Tout=tf.float32)
#     def convert_fn(q):
#         q_np = q.numpy()  # 这里可以用了，因为已经在 py_function 中
#         rotation = R.from_quat(q_np)
#         rot_matrix = rotation.as_matrix()
#         rot_6d = rot_matrix[:, :2, :].reshape(q_np.shape[0], 6)
#         return rot_6d.astype(np.float32)
    
#     return convert_fn(quat)
...
def lerobot_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # tf.print("proprio from data: ",trajectory["observation"]["proprio"])
...
OXE_STANDARDIZATION_TRANSFORMS = {
    # Downstream task
    "close_the_laptop": human_dataset_transform,
    "open_the_drawer": human_dataset_transform,
    "stack_the_red_bowl_on_top_of_the_blue_bowl": human_dataset_transform,
    "grasp_two_drinks_into_the_basket": human_dataset_transform,
    "put_the_cola_into_the_basket": human_dataset_transform,
    "open_the_drawer_15hz": human_dataset_transform,
    "open_the_drawer_and_put_the_bread": human_dataset_transform,
    "open_the_drawer_and_put_the_bread_10": human_dataset_transform,
    "open_the_drawer_and_put_the_bread_50": human_dataset_transform,
    "pick_place_the_apple_10": human_dataset_transform,
    "pick_place_the_apple_50": human_dataset_transform,
    "put_three_fruits_on_the_plate": human_dataset_transform,
    "open_the_drawer_and_put_the_glass_box": human_dataset_transform,
    "grasp_two_drinks_into_the_basket_1118": human_dataset_transform,
    "1120_demo_mix": human_dataset_transform,
    "open_the_drawer_joint": G1_dataset_transform,
    "egovla_rlds": human_dataset_transform,
    "water_flowers": human_dataset_transform,
    "water_flowers_15hz": human_dataset_transform,
    "pour_the_drinks_into_the_glass": human_dataset_transform,
    # Pretrain task
