...
from metis.util.data_utils import CollatorForMotionDynamics
from metis.vla.datasets import RLDSDatasetMotionDynamics, EpisodicRLDSDatasetMotionDynamics, RLDSBatchTransformVideoall_vision
...
class LightningOpenX(LightningDataset):
    ...
    def __init__(
        ...
        # vision_encoder
        self.batch_transform = RLDSBatchTransformVideoall_vision(
            image_transform=transforms.ToTensor()
        )

        self.collate_fn = CollatorForMotionDynamics(
            # vgg_transform=self.vggt
        )

        self.save_hyperparameters()

    def setup(self, stage: str) -> None:
        cls = RLDSDatasetMotionDynamics if not self.episodic else EpisodicRLDSDatasetMotionDynamics
        if stage == "fit":
