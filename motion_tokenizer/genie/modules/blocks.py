...
class SpatioTemporalBlock(nn.Module):
    ...
    def forward(self, x: Tensor, causal_temporal: bool = False, attn_mask: Tensor = None) -> Tensor:
        ...
        if attn_mask is not None:
            if attn_mask.dim() == 3:  # (B, T, S)
                # reshape: (B, T, S) -> (B*T, S)
                attn_mask_spatial = attn_mask.reshape(-1, attn_mask.shape[-1])
            elif attn_mask.dim() == 4:  # (B, T, S, S)
                # 4mask: (B, T, S, S) -> (B*T, S, S)
                attn_mask_spatial = attn_mask.reshape(-1, attn_mask.shape[-2], attn_mask.shape[-1])
            else:
                attn_mask_spatial = attn_mask
        else:
            attn_mask_spatial = attn_mask

        x_ = self.spatial_attn(x_, is_causal=False, attn_mask=attn_mask_spatial)
        x = x + x_
...
class SpatioBlock(nn.Module):
    ...
    def forward(self, x: Tensor, attn_mask: Tensor = None) -> Tensor:
        ...
        x_ = self.spatial_attn(x_, attn_mask=attn_mask_spatial)
        x = x + x_
        x = rearrange(x, "(b t) s e -> b t s e", t=t_len)
