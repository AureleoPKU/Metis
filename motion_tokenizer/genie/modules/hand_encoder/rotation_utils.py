...
def rotation_6d_to_rotation_vector(rot_6d):
    ...
    col2_proj = torch.sum(col2 * col1, dim=1, keepdim=True) * col1  # col1
    ...
    col3_norm = torch.norm(col3, dim=1, keepdim=True)
    col3 = col3 / (col3_norm + 1e-8)


    r11 = col1[:, 0:1, :, :, :]  # (N, 1, T, 1, 2)
    r12 = col2[:, 0:1, :, :, :]  # (N, 1, T, 1, 2)
    r13 = col3[:, 0:1, :, :, :]  # (N, 1, T, 1, 2)
    r21 = col1[:, 1:2, :, :, :]  # (N, 1, T, 1, 2)
    r22 = col2[:, 1:2, :, :, :]  # (N, 1, T, 1, 2)
    r23 = col3[:, 1:2, :, :, :]  # (N, 1, T, 1, 2)
    r31 = col1[:, 2:3, :, :, :]  # (N, 1, T, 1, 2)
    r32 = col2[:, 2:3, :, :, :]  # (N, 1, T, 1, 2)
    r33 = col3[:, 2:3, :, :, :]  # (N, 1, T, 1, 2)
    ...
    # Rodrigues
    rx = (r32 - r23) / (2 * sin_angle)
    ry = (r13 - r31) / (2 * sin_angle)
    rz = (r21 - r12) / (2 * sin_angle)
...
def test_6d_reconstruction_accuracy(rot_6d):
    ...
    col2_proj = torch.sum(col2 * col1, dim=1, keepdim=True) * col1
    col2_ortho = col2 - col2_proj
    col2_norm = torch.norm(col2_ortho, dim=1, keepdim=True)
    col2 = col2_ortho / (col2_norm + 1e-8)
