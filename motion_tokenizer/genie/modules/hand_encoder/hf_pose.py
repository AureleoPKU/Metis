...
from motion_tokenizer.genie.modules.hand_encoder.microaction_encoder import MicroactionEncoder
from motion_tokenizer.genie.modules.hand_encoder.ms_tcn_1D import MultiScale_TemporalConv as MS_TCN
from motion_tokenizer.genie.modules.hand_encoder.transformer_unimodal import Unimodal_TF
...
def quaternion_to_rotation_vector(quat):
    """Convert quaternions to rotation vectors (axis-angle).

    Args:
        quat: (N, 4, T, 1, 2) in [w, x, y, z] format.
    Returns:
        rot_vec: (N, 3, T, 1, 2) rotation vectors.
    """
        ...
        if self.use_global_wrist_reference:
            if self.include_orientation_in_global_wrist_ref:
                if self.use_both_wrists:
                    ...
                    xw = wrist[:, 0:3, :, 0, :]  # (N, 3, T, 1, 2) - xyz
                    wrist_quat = wrist[:, 3:9, :, 0, :]  # (N, 6, T, 1, 2) -

                else:
                    # For single hand, we only take the right hand wrist (as left hand is normalized by xf transformation).
                    xw = wrist[:, 0:3, :, 0, 1].unsqueeze(-1)  # (N, 3, T, 1, 2) - xyz
                    wrist_quat = wrist[:, 3:9, :, 0, 1].unsqueeze(-1)  # (N, 6, T, 1, 2) -
...
if __name__ == "__main__":
    ...
    model = HF_Pose(
                    microaction_window_size=4, num_joints=5,
                    embedding_dim_final=768, use_2d_pose=False, dropout=0, # [0.0, 0.99)
                    trajectory_atten_dim_per_head=4, trajectory_tcn_kernel_size=3, trajectory_tcn_stride=[1,2,2], trajectory_tcn_dilations=[1,2],
                    use_global_wrist_reference=True, include_orientation_in_global_wrist_ref=True, use_both_wrists=True, separate_hands=True,
                    tf_heads=8, tf_layers=2
                ).cuda()

    model.pose_tf = nn.Identity()
    model.classifier = nn.Identity()


    N, C, T, V, M = 1, 3, TOTAL_FRAMES, 5, 2

    x = torch.randn(N,C,T,V,M).cuda()
    wrist = torch.randn(N, 7, T, 1, 2).cuda()
    out=model(x, wrist)
    ...
    flops, params = profile(model, inputs=(x, wrist, ))
    ...
    gflop_dict, _ = flop_count(model, (x, wrist, ))
