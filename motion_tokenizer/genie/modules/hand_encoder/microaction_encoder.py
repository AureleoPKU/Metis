...
from motion_tokenizer.genie.modules.hand_encoder.ms_tcn_1D import MultiScale_TemporalConv
from motion_tokenizer.genie.modules.hand_encoder.trajectory_self_attention import Trajectory_SelfAttention # DETR-like self-attention
from motion_tokenizer.genie.modules.hand_encoder.utils import count_params
...
class MicroactionEncoder_graph_bak(nn.Module):
    def __init__(self, input_dim=3, embedding_dim=256, num_heads=[16,32,64], dropout=0.2, num_frames=15, \
                    num_joints=21, num_hands=2, stride=[1,2,2], kernel_size=3, dilations=[1,2], \
                    global_wrist_ref=False):
        super().__init__()
        ...
        num_nodes = num_joints * num_hands + (2 if global_wrist_ref else 0) # Number of tokens for self-attention with/without Global Wrist Token.

        self.data_bn = nn.BatchNorm1d(num_hands*(num_joints+1)*input_dim) # MVC, e.g., 2*21*3
    ...
    def forward(self, x, global_wrist_token=None):
        ...
        Args:
            x: [B, 16, 10, 64] input features.
            global_wrist_token: optional global wrist features with shape [B, 16, 2, 64].
        Returns:
            [B, 2, 10, 768] output features.
        """

        if global_wrist_token is not None:
            x = torch.cat([x, global_wrist_token], dim=2)

        x = x.permute(0, 3, 1, 2)  # [B, 16, 12, 64] -> [B, 64, 16, 12]
        N, C, T, J = x.shape  # [B, 64, 16, 12]
        ...
        # [B, 768, 2, 12] -> [B, 2, 10, 768]
        out = x[:,:,:,:self.num_joints*self.num_hands]
        out = out.permute(0, 2, 3, 1).contiguous()  # [B, 768, 2, 10] -> [B, 2, 10, 768]
        return out
